...
export default class cityComponent {
    ...
    //省份选择
    onProvinceListClick(e) {
        var _target = e.target,
            _citiesList = this.getCityChilds(_target.dataset.id);

        util.removeClass(document.querySelector('.city-active'), 'city-active');
        util.addClass(_target, 'city-active');
        this.DomMap.cityList.innerHTML = this.generateCityList(_citiesList);
        util.addClass(this.DomMap.cityListBox, 'city-box-show');
    }

    //城市选择
    onCityListClick(e) {
        var _target = e.target,
            _areaList = this.getAreaChilds(_target.dataset.id);

        util.removeClass(document.querySelector('.city-active'), 'city-active');
        util.addClass(_target, 'city-active');
        this.DomMap.areaList.innerHTML = this.generateCityList(_areaList);
        util.addClass(this.DomMap.areaListBox, 'city-box-show');
    }
    //地区选择
    onAreaListClick(e) {
        var _target = e.target,
            _province = this.DomMap.provinceList.querySelector('.city-active'),
            _city = this.DomMap.cityList.querySelector('.city-active'),
            _text = '',
            _params = {},
            cb = this.configMap.succCb;        //通过回调来和外部业务逻辑进行通讯

        if (_province === null || _city === null) return this.configMap.failCb();

        this.querySelector('.city-active') && util.removeClass(this.querySelector('.city-active'), 'city-active');
        util.addClass(_target, 'city-active');
        util.removeClass(DomMap.append_target, 'city-box-show');
        ...
        //
        stateMap.province_id = _province.dataset.id;
        stateMap.city_id = _province.dataset.id;
        stateMap.district_id = _target.dataset.id;

        typeof cb === 'function' && cb(_params);
    }

    onBackBtnClick() {
        let targetDom = this.DomMap.append_target,
            stateMap = this.stateMap,
            DomMap = this.DomMap;
        util.removeClass(targetDom, 'city-box-show');

        //未完成省市区所有选择时才进行处理
        if (!(stateMap.province_id && stateMap.city_id && stateMap.district_id)) {

            // 城市区域列表消失
            util.removeClass(DomMap.cityListBox, 'city-box-show');
            util.removeClass(DomMap.areaListBox, 'city-box-show');

            //清除样式
            util.removeClass(targetDom.getElementsByClassName('city-active')[0], 'city-active');
            util.removeClass(targetDom.getElementsByClassName('city-active')[0], 'city-active');

            stateMap.province_id = null;
            stateMap.city_id = null;
            stateMap.district_id = null;
        }
    }
    ...
    initModule(append_target) {
        ...
        //DOM绑定事件
        DomMap.provinceList.addEventListener('click', this.onProvinceListClick.bind(this));
        DomMap.cityList.addEventListener('click', this.onCityListClick.bind(this));
        DomMap.areaList.addEventListener('click', this.onAreaListClick.bind(this));
        //选择城市后退按钮
        DomMap.backBtn.addEventListener('click', this.onBackBtnClick.bind(this));
    }
}